...
exports.NavbarCSSUtil = void 0;
exports.NavbarCSSUtil = {
    /*
        CUSTOM CLASSES FOR ELEMENTS
    */
    simplifiedNavbar: ({ extractGaladrielClasses }) => {
        const genNavbar = ({ bgColorStart, bgColorEnd, textColor, hoverColor, shadowColor }) => ({
            "&": `{ background: linear-gradient(to top, ${bgColorEnd} 0%, ${bgColorStart} 100%); box-sizing: border-box; box-shadow: 0 6px 10px ${shadowColor}; display: flex; align-items: center; justify-content: center; user-select: none; position: fixed; top: 0; padding: 5px; height: 70px; width: 100%; z-index: 99; }`,
            ...
            "& li:hover ul": "{ transition: all 0.3s ease; top: 52px; margin: 0px -18px; opacity: 1; visibility: visible; }",
        });
