...
function transformNode(key, value, propertyKey = null) {
    // if its is a dynamic class
    if (!value.includes("$")) {
        // generates the class name
        const hashedProperty = (0, hashingHex_1.hashingHex)(JSON.stringify(`${key}:${value}`).replace(/\s+/g, ""));
        const className = `galadriel__${hashedProperty}${propertyKey ? `-g${(0, hashingHex_1.hashingHex)(propertyKey, false, true)}` : ""}`;
        return {
        ...
        const className = `${selector}${propertyKey ? `-g${(0, hashingHex_1.hashingHex)(propertyKey, false, true)}` : ""}`;
        return {
            transformedName: className,
        };
    }
...
function generatesTransformation(t, node) {
        ...
        if (property.value.type === "StringLiteral") {
            ...
            for (const nestedProperty of property.value.properties) {
                ...
                if (nestedProperty.value.type === "StringLiteral") {
                    // transform the property's value
                    const { transformedName } = transformNode(nestedProperty.key.name, nestedProperty.value.value, property.key.name);
                    // if the transformed name
                    if (transformedName) {
...
function transformAstNode(t, rootNode) {
    // loop stack
